...
bool Pipeline::apply_calcOffset(json_t *pStage, json_t *pStageModel, Model &model) {
    ...
    if (errMsg.empty()) {
        ...
        json_object_set_new(pStageModel, "channels", pChannels);
        json_object_set_new(pStageModel, "rects", pRects);

        json_t *pRect = json_object();
        json_array_append_new(pRects, pRect);
        json_object_set_new(pRect, "x", json_integer(roiScan.x + roiScan.width / 2));
        json_object_set_new(pRect, "y", json_integer(roiScan.y + roiScan.height / 2));
        json_object_set_new(pRect, "width", json_integer(roiScan.width));
        json_object_set_new(pRect, "height", json_integer(roiScan.height));
        json_object_set_new(pRect, "angle", json_integer(0));
        ...
        for (int iChannel = 0; iChannel < channels.size(); iChannel++) {
            ...
            json_object_set_new(pChannels, key, pMatches);
            if (matches.size() == 1) {
                int mx = matches[0].x;
                int my = matches[0].y;
                float val = result.at<float>(my, mx);
                if (minval <= val) {
                    int dx = xtol - mx;
                    int dy = ytol - my;
                    json_object_set_new(pMatches, "dx", json_integer(dx));
                    json_object_set_new(pMatches, "dy", json_integer(dy));
                    json_object_set_new(pMatches, "match", json_float(val));
                    if (dx || dy) {
                        json_t *pOffsetRect = json_object();
                        json_array_append_new(pRects, pOffsetRect);
                        json_object_set_new(pOffsetRect, "x", json_integer(roi.x + roi.width / 2 - dx));
                        json_object_set_new(pOffsetRect, "y", json_integer(roi.y + roi.height / 2 - dy));
                        json_object_set_new(pOffsetRect, "width", json_integer(roi.width));
                        json_object_set_new(pOffsetRect, "height", json_integer(roi.height));
                        json_object_set_new(pOffsetRect, "angle", json_integer(0));
                        if (!pOffsetColor) {
                            pOffsetColor = json_array();
                            json_array_append_new(pOffsetColor, json_integer(offsetColor[0]));
                            json_array_append_new(pOffsetColor, json_integer(offsetColor[1]));
                            json_array_append_new(pOffsetColor, json_integer(offsetColor[2]));
                        }
                    }
                }
            }
        ...
        json_array_append_new(pRects, pRoiRect);
        json_object_set_new(pRoiRect, "x", json_integer(roi.x + roi.width / 2));
        json_object_set_new(pRoiRect, "y", json_integer(roi.y + roi.height / 2));
        json_object_set_new(pRoiRect, "width", json_integer(roi.width));
        json_object_set_new(pRoiRect, "height", json_integer(roi.height));
        json_object_set_new(pRoiRect, "angle", json_integer(0));
        if (pOffsetColor) {
            json_object_set_new(pRoiRect, "color", pOffsetColor);
        }
