...
json_t * json_matrix(const Mat &mat) {
    ...
    for (int r = 0; r < mat.rows; r++) {
        for (int c = 0; c < mat.cols; c++) {
            json_array_append_new(jmat, json_real(mat.at<double>(r, c)));
        }
    }
    return jmat;
}
...
typedef struct GridMatcher {
    ...
    string identifyRows(json_t *pStageModel, vector<Point2f> &pointsXY, float &dyMedian, Point2f &dyTot1,
        ...
        string errMsg;
        json_object_set_new(pStageModel, "dyMedian", json_real(dyMedian));
        json_object_set_new(pStageModel, "dyCount1", json_integer(dyCount1));
        json_object_set_new(pStageModel, "dyCount2", json_integer(dyCount2));
        if (dyCount1 == 0) {
            errMsg = "No grid points matched within tolerance (level 1) dyCount1:0";
        } else if (dyCount2 == 0) {
            json_object_set_new(pStageModel, "dxAvg1", json_real(dyTot1.x / dyCount1));
            json_object_set_new(pStageModel, "dyAvg1", json_real(dyTot1.y / dyCount1));
            ...
            json_object_set_new(pStageModel, "dydxAvg1", json_real(dxAvg1));
            json_object_set_new(pStageModel, "dydyAvg1", json_real(dyAvg1));
            json_object_set_new(pStageModel, "dydxAvg2", json_real(dxAvg2));
            json_object_set_new(pStageModel, "dydyAvg2", json_real(dyAvg2));
            float normXY = sqrt(dxAvg2 * dxAvg2 + dyAvg2 * dyAvg2);
            grid.y = normXY / objSep.y;
            json_object_set_new(pStageModel, "grid.y", json_real(grid.y));
    ...
    string identifyColumns(json_t *pStageModel, vector<Point2f> &pointsYX, float &dxMedian, Point2f &dxTot1,
        ...
        string errMsg;
        json_object_set_new(pStageModel, "dxMedian", json_real(dxMedian));
        json_object_set_new(pStageModel, "dxCount1", json_integer(dxCount1));
        json_object_set_new(pStageModel, "dxCount2", json_integer(dxCount2));
        if (dxCount1 == 0) {
            errMsg = "No grid points matched within tolerance (level 1) dxCount1:0";
        } else if (dxCount2 == 0) {
            json_object_set_new(pStageModel, "dxAvg1", json_real(dxTot1.x / dxCount1));
            json_object_set_new(pStageModel, "dyAvg1", json_real(dxTot1.y / dxCount1));
            ...
            json_object_set_new(pStageModel, "dxdxAvg1", json_real(dxAvg1));
            json_object_set_new(pStageModel, "dxdyAvg1", json_real(dyAvg1));
            json_object_set_new(pStageModel, "dxdxAvg2", json_real(dxAvg2));
            json_object_set_new(pStageModel, "dxdyAvg2", json_real(dyAvg2));
            float normXY = sqrt(dxAvg2 * dxAvg2 + dyAvg2 * dyAvg2);
            grid.x = normXY / objSep.x;
            json_object_set_new(pStageModel, "grid.x", json_real(grid.x));
        ...
        for (int i = 0; i <= n + dr; i++) {
            for (int j = 0; j <= n + dc; j++) {
                if (0 <= r + i && r + i < gridIndexes.rows && 0 <= c + j && c + j < gridIndexes.cols) {
                    ...
                    if (0 <= index) {
                        if (pSubImgSet) {
                            pSubImgSet->insert(imagePts[index]);
                        }
                        pDst += imagePts[index];
    ...
    string calibrateImage(json_t *pStageModel, Mat &cameraMatrix, Mat &distCoeffs,
        ...
        json_object_set_new(pStageModel, "calibrate", pCalibrate);
        string errMsg;
        vObjectPts.clear();
        vImagePts.clear();
        ...
        if (errMsg.empty()) {
            ...
            for (int i = 0; i < size(); i++) {
                ...
                json_object_set_new(pRect, "x", json_real(imagePts[i].x));
                json_object_set_new(pRect, "y", json_real(imagePts[i].y));
                json_object_set_new(pRect, "objX", json_real(objSep.x * (objectPts[i].x)));
                json_object_set_new(pRect, "objY", json_real(objSep.y * (objectPts[i].y)));
                ...
                if (it == subImgSet.end()) { // color marks points not used in calibration
                    ...
                    json_object_set_new(pRect, "color", pBGR);
                    json_array_append_new(pBGR, json_integer(color[0]));
                    json_array_append_new(pBGR, json_integer(color[1]));
                    json_array_append_new(pBGR, json_integer(color[2]));
                }
            }
        ...
        json_object_set_new(pCalibrate, "perspective", json_matrix(perspective));
        json_object_set_new(pCalibrate, "op", json_string(opStr.c_str()));
        json_object_set_new(pCalibrate, "cameraMatrix", json_matrix(cameraMatrix));
        json_object_set_new(pCalibrate, "distCoeffs", json_matrix(distCoeffs));
        json_object_set_new(pCalibrate, "candidates", json_integer(size()));
        json_object_set_new(pCalibrate, "matched", json_integer(subImgSet.size()));
        json_object_set_new(pCalibrate, "images", json_real(vImagePts.size()));
        json_object_set_new(pCalibrate, "rmserror",
                            isnan(rmserror) ? json_string("NaN") : json_real(rmserror));
        json_t * jgridnessIn = json_array();
        json_object_set_new(pCalibrate, "gridnessIn", jgridnessIn);
        json_array_append_new(jgridnessIn,
        ...
        json_object_set_new(pCalibrate, "gridnessOut", jgridnessOut);
        json_array_append_new(jgridnessOut,
        ...
        json_object_set_new(pCalibrate, "rvecs", pRvecs);
        for (int i = 0; i < rvecs.size(); i++) {
            json_array_append_new(pRvecs, json_matrix(rvecs[i]));
        }
        json_t *pTvecs = json_array();
        json_object_set_new(pCalibrate, "tvecs", pTvecs);
        for (int i = 0; i < tvecs.size(); i++) {
            json_array_append_new(pTvecs, json_matrix(tvecs[i]));
        }
#endif
...
bool Pipeline::apply_undistort(const char *pName, json_t *pStage, json_t *pStageModel, Model &model) {
    ...
    if (errMsg.empty()) {
        if (distCoeffs.rows >= 4 && cameraMatrix.rows == 3) {
            json_object_set_new(pStageModel, "model", json_string(modelName.c_str()));
            Mat dst;
            InputArray newCameraMatrix = noArray();
            undistort(model.image, dst, cameraMatrix, distCoeffs, newCameraMatrix);
            model.image = dst;
        } else {
            json_object_set_new(pStageModel, "model", json_string(""));
        }
    }
