        ...
        if (split) {
            for (int i = 0; i < bins; i++) {
                ...
                for (int channel = 0; channel < histChannels.size(); channel++) {
                    ...
                    json_array_append_new(pDimHist, pNum);
                }
                if (keep) {
                    char numBuf[20];
                    snprintf(numBuf, sizeof(numBuf), "%d", i);
                    json_object_set_new(pHist, numBuf, pDimHist);
                } else {
            ...
            for (int i = 0; i < bins; i++) {
                ...
                if ((binValue == 0 || binMax && binValue > binMax)) {
                    ...
                    snprintf(numBuf, sizeof(numBuf), "%d", i);
                    json_t *pNum = (int)binValue == binValue ? json_integer(binValue) : json_real(binValue);
                    json_object_set_new(pHist, numBuf, pNum);
                }
            }
        }
        json_object_set_new(pStageModel, "hist", pHist);
        if (locations) {
            json_t *pLocations = json_array();
            json_object_set_new(pStageModel, "locations", pLocations);
        }
    }
